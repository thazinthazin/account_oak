<?php
	...
	$journal_item = [];
	$name = $journal_item["name"] = "P001";   // from REG form
	$totalamount = $journal_item["totalamount"] = 300000;   // from REG form (Course Fees)
	$user_payamount = $journal_item["income_amount"] = 100000;   // from REG form (Student Pay)
...
// print_r($journal_item);
	...
	if ($frequency == 1) {    
		$item_sql .= monthlyPay($link,$journal_item);
	...
	function monthlyPay($link,$journal_item) {
		$item_sql = "";
		...
		if ($journal_item["totalamount"] > $journal_item["income_amount"]) {
			...
			if (!HasLiabilityAmount($link,$journal_item["name"])) {
				$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `credit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
				...
				echo "Change liability Credit to Debit";
			}

		} else if ($journal_item["totalamount"] < $journal_item["income_amount"]) {
			$last_amount = $journal_item["income_amount"] - $journal_item["totalamount"];

			if (HasReceivableAmount($link,$journal_item["name"])) {
				$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `debit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
				VALUES ('$journal_item[name]','$journal_item[productid]','$last_amount','$journal_item[receivable_accountid]','$journal_item[journalentryid]','$journal_item[journalid]','$journal_item[paymentid]','$journal_item[invoiceid]');";
			} else {
				$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `credit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
				VALUES ('$journal_item[name]','$journal_item[productid]','$last_amount','$journal_item[lia_accountid]','$journal_item[journalentryid]','$journal_item[journalid]','$journal_item[paymentid]','$journal_item[invoiceid]');";
			}
		}
		return $item_sql;
	}
	...
	function prePay($journal_item,$frequency) {
		$item_sql = "";
	 	...
	 	if (($diff_amount > 0) && ($frequency != $new_frequency)) {
	 		$journal_item["totalamount"] = $journal_item["lia_amount"] = $journal_item["income_amount"];

	 		$item_sql .= prePay($journal_item,$new_frequency);
	 		// for receivable amount
	 		$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `credit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
			VALUES ('$journal_item[name]','$journal_item[productid]','$diff_amount','$journal_item[receivable_accountid]','$journal_item[journalentryid]','$journal_item[journalid]','$journal_item[paymentid]','$journal_item[invoiceid]');";
	 	} else {    // full pay
		 	for ($i=0; $i < $frequency; $i++) {
		 		// echo "<br>i - value =". $i;
				// income account
				$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `credit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
				VALUES ('$journal_item[name]','$journal_item[productid]','$actual_cost_per_month','$journal_item[income_accountid]','$journal_item[journalentryid]','$journal_item[journalid]','$journal_item[paymentid]','$journal_item[invoiceid]');";

				$journal_item["lia_amount"] -= $actual_cost_per_month;
				// liability account
				$item_sql .= "INSERT INTO `journal_item`( `name`, `productid`, `credit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`)
	...
	function HasReceivableAmount($link,$name) {
		$sql = "SELECT accountid,credit FROM `journal_item` WHERE `name` = '$name' ORDER BY id DESC LIMIT 1";
		...
		if ($result->num_rows > 0) {
		    // output data of each row
		    while($row = $result->fetch_assoc()) {
		        if (($row["accountid"] == 1) && ($row["credit"] > 0)) {
		        	return true;
		        }

		        echo "<br>Account ID is: " . $row["accountid"];
		        echo "<br>Credit Amount is: " . $row["credit"];
		    }
		}
		return false;
	}

	function HasLiabilityAmount($link,$name) {
		$sql = "SELECT accountid,credit FROM `journal_item` WHERE `name` = '$name' ORDER BY id DESC LIMIT 1";
		...
		if ($result->num_rows > 0) {
		    // output data of each row
		    while($row = $result->fetch_assoc()) {
		        if (($row["accountid"] == 5) && ($row["credit"] > 0)) {
		        	return true;
		        }
