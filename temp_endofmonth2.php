<?php
	...
	$sql = "SELECT entry.`name`,entry.`totalamount`,entry.`frequency`,entry.`id`,item.`debit` AS user_payment,item.`journalid`,entry.`userid`,item.`productid`,item.`paymentid`,item.`invoiceid` FROM `journal_item` AS item JOIN `journal_entry` AS entry ON item.`name` = entry.`name`
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
			...
			VALUES ('$row[name]','$row[productid]','$cost_per_month','$income_accountid','$row[id]','$row[journalid]','$row[paymentid]','$row[invoiceid]','$date');";

			// defer income amount
			$item_sql .= "INSERT INTO `journal_item`(`name`,`productid`,`debit`,`accountid`,`journalentryid`,`journalid`,`paymentid`,`invoiceid`,`createdon`)
			VALUES ('$row[name]','$row[productid]','$cost_per_month','$defer_incomeid','$row[id]','$row[journalid]','$row[paymentid]','$row[invoiceid]','$date');";
	    }
